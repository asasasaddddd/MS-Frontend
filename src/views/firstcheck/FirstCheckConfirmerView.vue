...
function openConfirm(row: ConfirmerRow) {
  activeRow.value = row
...
async function loadRows() {
  ...
  try {
    ...
    const confirmerNodeSet = new Set<string>(workflowNodeGroups.firstcheck.confirmer)
    const firstCheckTasks = tasks.filter((task) => {
      const pending = isPendingWorkflowTask(task)
      return pending && matchesBusinessType(task.businessType, 'firstcheck') && confirmerNodeSet.has(task.nodeCode) && matchesRouteOrder(task)
    })
    ...
    rows.value = []
    message.error(error instanceof Error ? error.message : '确认员首检待办加载失败')
  } finally {
    loading.value = false
  }
}
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-button type="link" class="button-link" @click="openConfirm(record)">处理</a-button>
          </template>
