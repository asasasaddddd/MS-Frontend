...
const emit = defineEmits<{
  ...
  submit: [payload: PeriodicResponsibleScrapTrackingDecisionRequest]
}>()
...
const decisionName = computed(() => (form.trackingRequired ? '需要不合格追踪' : '不需要不合格追踪'))
...
function submit() {
  ...
  emit('submit', {
    ...
    trackingRequired: form.trackingRequired,
...
watch(
  ...
  (open) => {
    if (!open) return
    form.trackingRequired = true
    form.opinion = '需要进行不合格追踪'
  }
)

watch(
  () => form.trackingRequired,
  (required, previous) => {
    if (!props.open) return
    const previousDefault = previous ? '需要进行不合格追踪' : '不需要进行不合格追踪'
    if (!form.opinion || form.opinion === previousDefault) {
      form.opinion = required ? '需要进行不合格追踪' : '不需要进行不合格追踪'
    }
  }
)
</script>
  ...
  <a-modal :open="open" width="760px" :footer="null" :closable="false" :destroy-on-close="true" @cancel="close">
    <template #title>
      <div class="dialog-title">
        <div>
          <div class="dialog-breadcrumb">首页 / 工作台 / 待办事项 / 不合格追踪判定</div>
          <strong>责任工程师判定是否进行不合格追踪</strong>
        </div>
    ...
    <div class="form-page">
      ...
      <section class="panel">
        <div class="panel-header">
          <h2>追踪判定</h2>
          <a-tag class="tag orange">{{ decisionName }}</a-tag>
        </div>
        <div class="form-grid">
          <label>
            <span>是否进行不合格追踪</span>
